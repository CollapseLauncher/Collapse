...
{
    private async ValueTask RepairAssetAudioType(
        ...
        CancellationToken    token)
    {
        ...
        if (!asset.GetAudioPatchUrlProperty(out ManifestAudioPatchInfo? patchInfo,
                                            out string? patchUrl))
        {
            throw new InvalidOperationException("Cannot get audio patch properties!");
        }

        string patchOutputPath = Path.Combine(GamePath,
                                              AssetBundleExtension.RelativePathAudioPatch,
        ...
        string oldFilePath = Path.Combine(GamePath,
                                          AssetBundleExtension.RelativePathAudio,
                                          patchInfo.AudioFilename);
        ...
        int  bufferSize = patchInfo.PatchFileSize.GetFileStreamBufferSize();
        long loaded     = 0;
                                              ...
                                              bufferSize))
        {
            ...
            if (IsBytesEqualReversible(patchInfo.PatchMD5Array, hashBuffer))
            {
                goto StartPatch;
            }
        }
        ...
        await RunDownloadTask(patchInfo.PatchFileSize,
                              patchOutputFileInfo,
                              patchUrl,
