...
set GenericCMAKEParamStaticNoFlagsNoIntOpt=-DCMAKE_BUILD_TYPE=%BuildType% %Toolchain% -DCMAKE_MSVC_RUNTIME_LIBRARY=%RuntimeLibrary%
set GenericCMAKEParamStaticNoFlags=%GenericCMAKEParamStaticNoFlagsNoIntOpt% -DCMAKE_INTERPROCEDURAL_OPTIMIZATION=true
set GenericCMAKEParamStatic=%GenericCMAKEParamStaticNoFlags% -DCMAKE_CXX_FLAGS="%CXXFLAGS%" -DCMAKE_C_FLAGS="%CFLAGS%" -DCMAKE_EXE_LINKER_FLAGS="%LDFLAGS%" -DCMAKE_EXE_LINKER_FLAGS_INIT="%LDFLAGS%"
...
:Build_waifu2x
    ...
    rmdir /S /Q vclatestbuild
    cmake . %GenericCMAKEParamStatic% -B vclatestbuild -DCMAKE_POLICY_VERSION_MINIMUM=3.5 || goto :ERR
    cd vclatestbuild
...
:Build_libomp
    ...
    cd runtimes
    rmdir /S /Q vclatestbuild
    cmake . %GenericCMAKEParamStaticNoFlagsNoIntOpt% -DENABLE_CHECK_TARGETS=FALSE -DLLVM_ENABLE_RUNTIMES=openmp -DLIBOMP_ENABLE_SHARED=FALSE -B vclatestbuild || goto :ERR
    cd vclatestbuild
    msbuild ALL_BUILD.vcxproj -p:Configuration=%BuildType% /m:%NUMBER_OF_PROCESSORS% || goto :ERR
...
:Build_libwebp
    ...
    git fetch --all && git pull --all
    rmdir /S /Q vclatestbuild
    cmake . %GenericCMAKEParamStatic% -DBUILD_SHARED_LIBS=0 -DWEBP_LINK_STATIC=1 -DEMSCRIPTEN=OFF -DWEBP_USE_THREAD=ON ^
...
:Build_libheif
    ...
    :: git fetch --all && git pull --all
    :: rmdir /S /Q vclatestbuild
    :: cmake . %GenericCMAKEParamStatic% -DZLIB_BUILD_STATIC=1 -DZLIB_BUILD_SHARED=0 -B vclatestbuild
    :: cd vclatestbuild
    ...
    if /I not "%Toolchain%" EQU "" (
        ...
        set CXXFLAGS=%CXXFLAGS% -msse4.1 -mssse3 -msse2 -EHsc
        set CFLAGS=%CFLAGS% -msse4.1 -mssse3 -msse2 -EHsc
    ) else (
        set OLD_CXXFLAGS=%CXXFLAGS%
        set OLD_CFLAGS=%CFLAGS%
    ...
    :: cmake . %GenericCMAKEParamStaticNoFlags% -DBUILD_SHARED_LIBS=0 -DENABLE_SDL=OFF -B vclatestbuild ^
    ...
    rmdir /S /Q vclatestbuild
    :: cmake . %GenericCMAKEParamStatic% --preset=release-noplugins -DBUILD_SHARED_LIBS=0 -B vclatestbuild -DENABLE_EXPERIMENTAL_FEATURES=1 -DWITH_REDUCED_VISIBILITY=1 ^
    ...
    cmake . %GenericCMAKEParamStaticNoFlags% --preset=release-noplugins -DCMAKE_CXX_FLAGS="%CXXFLAGS%" -DCMAKE_C_FLAGS="%CFLAGS%" ^
    -DBUILD_SHARED_LIBS=0 -B vclatestbuild -DENABLE_EXPERIMENTAL_FEATURES=0 -DWITH_REDUCED_VISIBILITY=1 ^
    -DWITH_LIBDE265=1 -DLIBDE265_INCLUDE_DIR=..\libde265 -DLIBDE265_LIBRARY=%thisdir%\libde265\vclatestbuild\libde265\%BuildType%\libde265.lib ^
    -DWITH_DAV1D=1 -DDAV1D_INCLUDE_DIR=..\dav1d\include -DDAV1D_LIBRARY=%thisdir%\dav1d\build\src\libdav1d.lib ^
    -DWITH_HEADER_COMPRESSION=0 -DWITH_UNCOMPRESSED_CODEC=0 || goto :ERR
...
:Build_libjxl
    ...
    git fetch --all && git pull --all
    rmdir /S /Q vclatestbuild
    cmake . %GenericCMAKEParamStaticNoFlags% -DCMAKE_CXX_FLAGS="%CXXFLAGS%" -DCMAKE_C_FLAGS="%CFLAGS%" -DBUILD_TESTING=OFF -DJPEGXL_STATIC=1 -B vclatestbuild || goto :ERR
    cd vclatestbuild
    msbuild ALL_BUILD.vcxproj -p:Configuration=%BuildType% /m:%NUMBER_OF_PROCESSORS% || goto :ERR
...
:Build_libzstd
    ...
    git fetch --all && git pull --all
    rmdir /S /Q vclatestbuild
    cmake . %GenericCMAKEParamStatic% -DBUILD_TESTING=OFF -DZSTD_USE_STATIC_RUNTIME=ON -DZSTD_BUILD_STATIC=ON -DZSTD_BUILD_PROGRAMS=OFF -DZSTD_MULTITHREAD_SUPPORT=ON -B vclatestbuild || goto :ERR
