...
namespace Hi3Helper.DiscordPresence
{
    ...
    public class DiscordPresenceManager : IDisposable
    {
        ...
        private void EnablePresence(long applicationId)
        {
            ...
            {
                if (_client != null) DisablePresence();
            ...
            _client.OnReady += (_, msg) =>
            {
                ...
                else
                {
                    ...
                    if (!(!_cachedIsIdleEnabled && (_activityType == ActivityType.Idle || _activityType == ActivityType.None)))
                    {
                    ...
                    default:
                        _presence = new RichPresence
                        {
                            ...
                            Assets = new Assets()
                            {
                                ...
                                LargeImageText = $"Collapse Launcher v{AppCurrentVersionString} {(IsPreview ? "Preview" : "Stable")}"
                            }
                        };
                        break;
                }
